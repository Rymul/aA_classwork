...
# original key when given to the block. Each value of the new hash should be the result of 
...
# then it should be replaced with a question mark ('?'). Space characters (' ') should remain 
# unchanged.
...
def convert_pig_latin(sentence)
    ...
    pig = new_arr.join(" ")
    #final = pig[0].upcase + pig[1..-1].downcase
end
...
def change(word)
    ...
    word.each_char.with_index do |char, i|
        if vowels.include?(char.downcase) && word[0] == word[0].upcase
            return word[i].upcase + word[i + 1..-1] + word[0...i] + "ay"
